<template>
  <div>
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body" style="text-align: justify">
            ...
            <p>
              Input a query in the search field. Every time you press Space or
              Enter after a word, a <b>token</b> will be created. The token is
              composed of a <b>prefix</b> describing how the query should be
              treated and the actual <b>query</b> itself.
            </p>

            <strong>What are the different types of tokens?</strong>
            <p>
              Each token represents either a Gene, HOG, OMA group, or Taxon.
              Prefixes are used to specify which category to associate with the
              query term.
            </p>
            ...
            <small>
              For example, the token [go:4225] will search for genes in the OMA
              database annotated with the GO:0004225 gene ontology term.
            </small>
            <br />

            <strong>How to search for a multi-word query?</strong>
            <p>
              If your query term is composed of multiple words (e.g. homo
              sapiens), use " " to encapsulate it.
            </p>

            <strong>How many tokens can I have?</strong>
            <p>
              There is no limit on the number of tokens. It is not possible to
              enter multiple tokens of different categories, except taxon, which
              can be combined with other categories. For example, you can search
              for 'hog:60627 species:HUMAN' to return human genes found in
              HOG:606207.
            </p>

            <strong>How to edit/delete a token?</strong>
            <p>
              To edit a query, click on it to modify the input field. To edit a
              prefix, click on the dropdown icon to select another one. To
              remove a token, click on the x to delete it.
            </p>

            <strong>Autosuggest</strong>
            <p>
              Typing a query without hitting enter or space will prompt an
              autosuggestion for the identifier after a few seconds.
            </p>
          </div>
    ...
    <form method="POST" :id="`form_${uniqueId}`" :action="searchUrl">
      ...
      <div
        :id="`${uniqueId}_input_part`"
        :class="['search-input-container', multiline ? 'ml-input' : 'sl-input']"
      >
        <!-- Help Icon -->
        ...
        <!-- Token Container -->
        <div
          :id="`token-container_${uniqueId}`"
          :class="[
            'token-container',
            multiline ? 'ml-token-con' : 'sl-token-con',
          ]"
        >
          <span
          ...
          >
            <select
              ...
              @change="onChangePrefix($event.target, token)"
            >
              <template
                v-for="(prefixesList, prefixType) in prefixes"
                :key="prefixType"
              >
...
const placeholder = ref(
  'proteinid:P53_RAT | "Blue-light photoreceptor" | species:"Drosophila melanogaster"'
);
...
function addToken(event) {
  ...
  const tokenContainer = document.getElementById(
    `token-container_${props.uniqueId}`
  );
  if (tokenContainer) {
...
function detokenize(index) {
  ...
  const inputTokenSearch = document.getElementById(
    `input_token_search_${props.uniqueId}`
  );
  ...
  if (typeof $ !== "undefined" && $.fn.autocomplete) {
    const autocompleteOpts = {
      ...
      formatResult: function (suggestion, currentValue) {
        ...
        return (
          '<span class="auto-xref">' +
          highlightAndEscape(suggestion.data.xref) +
          "</span> &nbsp;" +
          '<span class="auto-omaid">' +
          suggestion.data.omaid +
          "</span>" +
          '<span class="auto-species">' +
          highlightAndEscape(suggestion.data.genome.species) +
          "</span>"
        );
      },
